...
所有工具通过 `DeviceCapabilityBridge` facade 访问系统能力。测试中用 `FakeDeviceCapabilityBridge` 组合多个 `RecordingXxxBridge` 替代真实 Android 实现。每个 Recording Bridge 会记录被调用的方法和参数，并返回可配置的结果。
...
| `FakeDeviceCapabilityBridge.kt` | `FakeDeviceCapabilityBridge`, `StubAppStateBridge`, `StubAccessibilityBridge`, `StubServiceGatewayForTest` | 组合所有 Recording Bridge 成一个完整的测试用 `DeviceCapabilityBridge` |
...
fun myNewTool_callsExpectedBridge() = runBlocking {
    // 1. 创建 FakeDeviceCapabilityBridge（内含所有 Recording Bridge）
...
| `core/domain/src/test/.../testdoubles/FakeDeviceCapabilityBridge.kt` | 组合型测试 Bridge + Stub |
